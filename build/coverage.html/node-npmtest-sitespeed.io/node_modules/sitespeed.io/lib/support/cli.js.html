...
<html lang="en">
<head>
    <title>Code coverage report for node-npmtest-sitespeed.io/node_modules/sitespeed.io/lib/support/cli.js</title>
    <meta charset="utf-8">
    <style>
        body, html {
            margin:0; padding: 0;
...
<div class="header low">
    ...
    <h1>Code coverage report for <span class="entity">node-npmtest-sitespeed.io/node_modules/sitespeed.io/lib/support/cli.js</span></h1>
    <h2>
    ...
    <div class="path"><a href="../../../../../index.html">All files</a> &#187; <a href="index.html">node-npmtest-sitespeed.io/node_modules/sitespeed.io/lib/support/</a> &#187; cli.js</div>
</div>
<div class="body">
...
&nbsp;
module.exports.parseCommandLine = <span class="fstat-no" title="function not covered" >function parseCommandLine() {</span>
<span class="cstat-no" title="statement not covered" >  let parsed = yargs</span>
    .env('SITESPEED_IO')
    .usage('$0 [options] &lt;url&gt;/&lt;file&gt;')
    .require(1, 'urlOrFile')
    .version(() =&gt; <span class="cstat-no" title="statement not covered" >`${packageInfo.name} ${packageInfo.version}`)</span>
    .option('debug', {
      default: false,
...
&nbsp;
  ...
  .option('webpagetest.host', {
    ...
    */
    ...
    .config('config')
    .alias('version', 'V')
    .coerce('budget', <span class="fstat-no" title="function not covered" >function(arg) {</span>
<span class="cstat-no" title="statement not covered" >      if (typeof arg === 'object' &amp;&amp; !Array.isArray(arg)) {</span>
<span class="cstat-no" title="statement not covered" >        if (arg.configPath) {</span>
<span class="cstat-no" title="statement not covered" >          arg.config = JSON.parse(fs.readFileSync(arg.configPath, 'utf8'));</span>
        } else <span class="cstat-no" title="statement not covered" >if (arg.config){</span>
<span class="cstat-no" title="statement not covered" >          arg.config = JSON.parse(arg.config);</span>
        }
<span class="cstat-no" title="statement not covered" >        return arg;</span>
      } else {
<span class="cstat-no" title="statement not covered" >        throw new Error('[ERROR] Something looks wrong with your budget configuration. Since sitespeed.io 4.4 you should pass the path to your budget file through the --budget.configPath flag instead of directly through the --budget flag.');</span>
      }
    })
    .coerce('webpagetest', <span class="fstat-no" title="function not covered" >function(arg) {</span>
      // for backwards compatible reasons we check if the passed parameters is a path to a script, if so just us it (PR #1445)
<span class="cstat-no" title="statement not covered" >      if (arg.script &amp;&amp; fs.existsSync(arg.script)) {</span>
<span class="cstat-no" title="statement not covered" >        arg.script = fs.readFileSync(path.resolve(arg.script), 'utf8');</span>
        /* eslint no-console: off */
<span class="cstat-no" title="statement not covered" >        console.log('[WARNING] Since sitespeed.io 4.4 you should pass the path to the script file through the --webpagetest.file flag (https://github.com/sitespeedio/sitespeed.io/pull/1445).');</span>
<span class="cstat-no" title="statement not covered" >        return arg;</span>
      }
&nbsp;
<span class="cstat-no" title="statement not covered" >      if (arg.file) {</span>
<span class="cstat-no" title="statement not covered" >        arg.script = fs.readFileSync(path.resolve(arg.file), 'utf8');</span>
      } else <span class="cstat-no" title="statement not covered" >if (arg.script) {</span>
        // because the escaped characters are passed re-escaped from the console
<span class="cstat-no" title="statement not covered" >        arg.script = arg.script.split('\\t').join('\t');</span>
<span class="cstat-no" title="statement not covered" >        arg.script = arg.script.split('\\n').join('\n');</span>
      }
<span class="cstat-no" title="statement not covered" >      return arg;</span>
    })
...
&nbsp;
<span class="cstat-no" title="statement not covered" >  const aliases = parsed.getOptions().alias,</span>
    argv = parsed.argv;
&nbsp;
  // aliases are long options -&gt; short option
<span class="cstat-no" title="statement not covered" >  const aliasLookup = reduce(aliases, (lookup, value, key) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >    lookup.set(value[0], key);</span>
<span class="cstat-no" title="statement not covered" >    return lookup;</span>
  }, new Map());
&nbsp;
<span class="cstat-no" title="statement not covered" >  let explicitOptions = yargs.reset().argv;</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >  explicitOptions = reduce(explicitOptions, (result, value, key) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >    if (aliasLookup.has(key)) {</span>
<span class="cstat-no" title="statement not covered" >      const fullKey = aliasLookup.get(key);</span>
<span class="cstat-no" title="statement not covered" >      result = set(result, fullKey, value);</span>
    }
<span class="cstat-no" title="statement not covered" >    result = set(result, key, value);</span>
<span class="cstat-no" title="statement not covered" >    return result;</span>
  }, {});
&nbsp;
<span class="cstat-no" title="statement not covered" >  if (argv.config) {</span>
<span class="cstat-no" title="statement not covered" >    const config = require(path.resolve(process.cwd(), argv.config));</span>
<span class="cstat-no" title="statement not covered" >    explicitOptions = merge(explicitOptions, config);</span>
  }
&nbsp;
<span class="cstat-no" title="statement not covered" >  if (argv.webpagetest.custom) {</span>
<span class="cstat-no" title="statement not covered" >    argv.webpagetest.custom = fs.readFileSync(path.resolve(argv.webpagetest.custom), {</span>
      encoding: 'utf8'
    });
  }
&nbsp;
<span class="cstat-no" title="statement not covered" >  if (argv.summaryDetail) <span class="cstat-no" title="statement not covered" >argv.summary = true;</span></span>
&nbsp;
